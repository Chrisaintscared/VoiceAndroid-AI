...
from app.database import (
    ...
    get_pending_requests_for_student,   # ← NEW
...
def get_my_pending_requests(user=Depends(get_current_user)):
    if user["role"] != "student":
        raise HTTPException(status_code=403, detail="Students only")
    try:
        rows = get_pending_requests_for_student(user["id"])
        return [dict(r) for r in rows]
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to load pending requests: {str(e)}"
        )


# ── Teacher: all pending requests across their classes ─────────────────────────
@router.get("/requests")
def get_all_my_requests(user=Depends(get_current_user)):
    if user["role"] != "teacher":
        raise HTTPException(status_code=403, detail="Teachers only")
    try:
        rows = get_pending_requests_for_teacher(user["id"])
        return [dict(r) for r in rows]
    except Exception as e:
...
def get_class_requests(class_id: int, user=Depends(get_current_user)):
    ...
    try:
        rows = get_pending_requests(class_id)
        return [dict(r) for r in rows]
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to load requests: {str(e)}")
